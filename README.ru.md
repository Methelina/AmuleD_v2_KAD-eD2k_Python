...
.\AmuleD_Run.ps1 serve                # входящий peer-listener + upload-движок: раздаёт ваши файлы (Ctrl+C — остановить)
...
`serve` запускает входящий peer-listener как сервис: принимает eD2K client-to-client соединения, проводит handshake HELLO/HELLOANSWER, находит запрошенные хеши среди расшаренных файлов, ставит пиров в очередь (приоритеты, слоты, TTL, дедупликация) и отдаёт части файла с троттлингом на сессию:

```powershell
.\AmuleD_Run.ps1 serve                      # listener + периодическая KAD-перепубликация
.\AmuleD_Run.ps1 serve --publish-limit 10   # ограничить файлов на проход публикации
.\AmuleD_Run.ps1 serve --no-publish         # только listener
```

Демон рекламирует фактически занятый TCP-порт внутри KAD source-записей, пишет статус-снимок `db\serve_status.json` (pid, порт, активные соединения), соблюдает `serve.max_sessions` и корректно завершается по Ctrl+C. Идентичность клиента (userhash, ник, TCP-порт) живёт в секции `identity` файла `config\amuled.jsonc` — один и тот же userhash работает и в HELLO-handshake, и в KAD-публикации источников, как у eMule (`GetClientHash = GetUserHash`); при первом запуске userhash генерируется и сохраняется. Loopback-самотест: собственный загрузчик AmuleD забирает реальный файл у демона, и MD4 собранного совпадает.
...
├── AmuleD_Run.ps1             # Единый рантайм-диспетчер: меню / паук / serve / CLI
...
├── scripts/                   # KAD-паук, serve-демон, интерактивное меню, прогрев и диагностические скрипты
├── src/amuled_v2/             # Реализация на Python
│   ├── core/kad/              # KAD-движок (packets, bootstrap, routing, search, publish, obfuscation, strategies)
...
3. Каркас Credits / SecureIdent - **PLANNED**
4. Ротация протухших узлов паука, тактики поиска как CLI-флаги - **PLANNED**
5. GeoIP / UPnP-NAT-PMP - **PLANNED**

Заметки ранних сессий хранятся для контекста в docs/roadmap.md - каждая секция там помечена DONE/SOLVED/WIP/DEPRECATED/TODO; живое состояние - в секциях 11a-11f.
